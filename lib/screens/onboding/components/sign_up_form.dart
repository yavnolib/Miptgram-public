...
class SignUpForm extends StatefulWidget {
  ...
  SignUpForm({required this.prefs});

  @override
  State<SignUpForm> createState() => _SignUpFormState(prefs: prefs);
}

class _SignUpFormState extends State<SignUpForm> {
  final SharedPreferences prefs;
  _SignUpFormState({required this.prefs});
  ...
  Future<void> PostgresFuture() async {
    ...
    String surname_name_group = name_controller.text;
    ...
    if (_formKey.currentState!.validate()) {
      ...
      String surname = surname_name_group.split(' ')[0];
      String name = surname_name_group.split(' ')[1];
      String group = surname_name_group.split(' ')[2];
      print('name is $name');
  ...
  void signUpTest(BuildContext context) {
    setState(() {
      isShowConfetti = true;
      isShowLoading = true;
    });
  ...
  void signUp(BuildContext context, {required prefs}) {
    setState(() {
      isShowConfetti = true;
      isShowLoading = true;
    });
          ...
          Form(
            ...
            child: Column(
              ...
              children: [
                // name added
                const Text(
                  "Введите фамилию, имя и группу через пробел.",
                  style: TextStyle(
                ...
                Padding(
                  ...
                  child: TextFormField(
                    ...
                    validator: (value) {
                      if (value!.isEmpty) {
                        return "Введите фамилию, имя и группу через пробел.";
                      }
                      if (value.split(" ").length == 3) {
                        var res = value.split(" ");
                        if ((RegExp(r'[а-яА-Я]').allMatches(res[0]).length ==
                                res[0].length) &&
                            ((RegExp(r'[а-яА-Я]').allMatches(res[1]).length ==
                                res[1].length)) &&
                            (RegExp(r'[-А-Я0-9]').allMatches(res[2]).length ==
                                res[2].length) &&
                            (RegExp(r'[А-Я]').allMatches(res[2][0]).length ==
                                1)) {
                          return null;
                        }
                        return "Введите фамилию, имя и группу через пробел.";
